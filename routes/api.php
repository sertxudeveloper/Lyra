...
use SertxuDeveloper\Lyra\Http\Controllers\AssetsController;
...
Route::prefix(config('lyra.routes.api.prefix'))
  ->name(config('lyra.routes.api.name'))->group(function () {
    ...
    Route::middleware(['web', 'lyra-api'])->group(function () {
      /** Dashboard routes */
    ...
    Route::get('/styles/{name}', [AssetsController::class, 'style']);
    Route::get('/scripts/{name}', [AssetsController::class, 'script']);
    Route::get('/assets/{name}', [AssetsController::class, 'asset']);
