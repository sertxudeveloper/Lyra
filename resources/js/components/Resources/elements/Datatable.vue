<template>
  <div class="bg-white flex flex-col max-w-0 min-w-full rounded-lg shadow w-full">
    <div class="flex items-center justify-between h-9 m-4">
      ...
      <div class="flex items-center space-x-4 h-full">
        <!-- Actions tool -->
        <div class="flex h-full space-x-2" v-if="selected.length">
          <div class="bg-white border border-gray-300 focus-within:border-blue-500 focus-within:outline-none focus-within:ring-blue-500 rounded-md shadow-sm pr-1.5">
            <select v-model="actionSelected" class="bg-transparent block h-full outline-none px-2 py-1 sm:text-sm text-gray-600 w-56">
...
export default {
  ...
  methods: {
    ...
    runAction() {
      if (!this.actionSelected.trim()) return
  ...
  computed: {
    selectAll: {
      get() {
        return this.selected.length === this.resources.data.length;
      },
      set(value) {
        let selected = [];
